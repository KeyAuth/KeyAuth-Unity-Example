...
namespace KeyAuth
{
    public class api : MonoBehaviour
    {
        ...
        public string GetLeaderboard(string webid)
        {
            if (!initzalized)
            {
                error("Please initzalize first");
            ...
            if (json.success)
            {
                var DecodedMessage = JsonConvert.DeserializeObject<api.msg>(json.response);
                var Array = DecodedMessage.message;

                if (Array == null || string.IsNullOrWhiteSpace(Array))
                {
                    return null;
                }
                else
            ...
            {
                if (json.message == "Variable not found")
                {
                    Console.WriteLine("To use Leaderboard you need to create Public Variable named 'leaderboard'");
                    Console.ReadLine();
                    return null;
